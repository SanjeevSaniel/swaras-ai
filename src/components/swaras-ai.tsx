...
const SwarasAI = () => {
  ...
  const chatApi = useChat({
    ...
    onFinish: (message) => {
      ...
      // Fetch updated usage after message
      fetchUserUsage();
    },
    onError: async (error) => {
  ...
  // Fetch user usage on mount and after messages
  const fetchUserUsage = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setUserUsage(usage);
        setIsRateLimited(usage.remaining <= 0);
        logger.log('📊 User usage fetched:', usage);
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // Note: currentConversation is excluded - only re-run when conversation ID changes
  ...
  useEffect(() => {
    ...
    // Note: setMessages is a stable function from AI SDK useChat hook
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    ...
    if (!existingConversation) {
      ...
      try {
        ...
        // @ts-ignore - messageCount is optional in Conversation type
      ...
      return (
        <motion.div
          ...
          <ChatInput
            ...
            selectedPersona={selectedPersona}
            disabled={
              !mentorsOnline || mentorsLoading || isTyping || isRateLimited
            }
            isLoading={isTyping}
            userUsage={userUsage}
        ...
        <div className='flex h-full overflow-hidden relative z-10'>
          ...
          <div className='hidden lg:block flex-shrink-0 overflow-hidden'>
            <AppSidebar
              ...
              onSelectConversation={(convOrId: any) => {
                // Handle both string ID and Conversation object
                if (typeof convOrId === 'string') {
                  ...
                  setCurrentConversation(convOrId);
                }
              }}
              onNewConversation={() => {
                if (!selectedPersona || !mentorsOnline || mentorsLoading) {
              ...
              onDeleteConversation={(personaIdOrConv: any) => {
                // Handle both string persona ID and other types
                if (typeof personaIdOrConv === 'string') {
                  deleteConversation(personaIdOrConv);
                } else if (personaIdOrConv?.id) {
                  deleteConversation(personaIdOrConv.id);
                }
              }}
              selectedPersona={selectedPersona}
